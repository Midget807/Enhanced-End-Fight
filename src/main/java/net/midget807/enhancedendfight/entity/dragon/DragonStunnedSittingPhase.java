...
public class DragonStunnedSittingPhase extends AbstractDragonPhaseInstance implements NoMeleeDamage {
    public int stunnedDuration = 200;
    ...
    public DragonStunnedSittingPhase(EnderDragon dragon) {
        super(dragon);
    ...
    public void doServerTick() {
        stunnedTime++;
        if (this.dragon.getPhaseManager().getCurrentPhase() == EnderDragonPhase.DYING) {
            return;
        }
    ...
    @Override
    public EnderDragonPhase<DragonStunnedSittingPhase> getPhase() {
        return ModEnderDragonPhases.STUNNED_SITTING;
    }
